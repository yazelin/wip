...
非目標(YAGNI,留待 v3/之後):SessionStart hook(讓 Claude 開場自動跑 `wip --md`)、prebuilt binary 跨平台發布、`clear-done` 指令、可設定的 NEXT.md 路徑。**無新增 dependency。**
...
- 在組 `RepoStatus` 時多一步:`next_actions: next::read_open(repo)`。其餘不變。
...
### 5. `src/render.rs`(改)
- term:每 repo 區塊,若 `next_actions` 非空,多一行 `  next: 1. <text>   2. <text>`(numbered、空白分隔)。
- markdown:若非空,多一段
  ```
  - next:
    1. <text>
    2. <text>
  ```
- json:`next_actions` 已在序列化內,自動帶出。

### 6. `src/model.rs` — **不變**(`next_actions: Vec<String>` v1 已有)。
...
- `render`:`next_actions` 非空時 term/md 出現 numbered next 行;空時不出現。
- collector:NEXT.md 有開放項的 fixture repo → `RepoStatus.next_actions` 被填。
