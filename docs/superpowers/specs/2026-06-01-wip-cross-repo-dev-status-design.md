...
同時開發很多個 repo 時,追進度的方式往往是「進個別 repo 讀 git log」,沒有單一聚合視圖,於是常「忘記某條線做到哪」(典型觸發點:跨 repo 的整合工作做到一半,忘了停在哪)。
...
| 實作 | **Rust 單一 binary** | 跨機(Linux+Windows)、Claude 在哪台都能可靠叫同一支、v2 接 exchange 乾淨 |
| repo 範圍 | **curated 清單(config)**,非自動掃全部 | repo 一多會噪音爆炸;只追 active 的 |
| 住哪 | **新開獨立 github repo `wip`** | 它是管理一堆 repo 的 meta 工具,不塞進任何被它追蹤的既有 repo |
| 共享狀態(v2) | 走現成 **`exchange`** 信箱 | 人和 Claude 都讀得到;不另造儲存,避免又一個過時點 |

非目標(YAGNI):不做 web UI、不做多人協作、不自動掃全機 git repo、v1 不碰 AI session 事件。
...
### 1. config — repo 清單
- 設定檔列出要追的 repo 路徑(curated),種子放使用者當下 active 的幾個 repo(範例見 `repos.example.toml`)。
- 位置:`~/.config/wip/repos.toml`(可跨機同步)。
